...
I decided to write this in large part to document what I did so that I can work on my website in the future, and __blogdown__ + Hugo can be [finicky as #$*!](https://yihui.org/en/2018/02/marie-dussault/#:~:text=BTW) but hopefully this post can save you some pain and frustration if you are also a first-time user. 
...
A major thing that did not work for me was that __plots from .Rmd code chunks__ were not rendering. [Apparently __blogdown__ auto-detects and moves them](https://bookdown.org/yihui/blogdown/dep-path.html) to the `static/` folder, so I suspect they weren't rendering because the folder structure changed.  Though it could just have been user error or some outdated settings.  Or the fact that my computer is mostly duct tape, [pasta](https://twitter.com/alienanthology/status/1172190147665911808), and a hamster wheel.  
...
1. [post by Matteo Courthoud](https://matteocourthoud.github.io/post/website/) on making a website
    - This is the guide I started off using first, leading to a rebuild.  As such, I don't recommend using this --- some of the specifics seem to be outdated.  __Follow Alison Hill's slides instead__.  I _did_, however, find it helpful to have already read Matteo's guide as I started my second attempt.  For example, the [section on Basic Customization](https://matteocourthoud.github.io/post/website/#basic-customization) is helpful and specific.  Also, his color scheme is awesome and I totally copied it!
...
:::fyi
Alison Hill recommends [building the site locally, then dragging/dropping](https://arm.rbind.io/slides/blogdown.html#21) the `public` folder into Netlify.  Apart from being an extra few steps --- [or just one step](https://matteocourthoud.github.io/post/website/#automation) --- the main disadvantage is that this won't let you take advantage of __netlify's deploy branches__ option to test changes you make to the website before putting them out into the world.
:::
...
__Steps:__  
  ...
  ![staging a large folder in RStudio's Git   tab](https://media.giphy.com/media/qNQFUVIKAt9Ek/giphy.gif "...or debugging a hugo website")
  ...
  1. Your url will be something oddly delightful like "youthful-elephant-g0at33.netlify.app".  Change it: "Site settings" link --> "Change site name".  
      + You can always change the site name, or set up hosting on a new domain.  [`rbind.io` is a recommended, and free, option](https://support.rbind.io/about/).

Cool!  Now whenever you push to your repo, netlify will build it and serve your website.


:::note
Use the branch deploy feature to preview changes (such as a new post)!  All you have to do now is create a branch __AND make a pull request__, and netlify will let you preview it!  [More details + suggested workflow here](https://www.garrickadenbuie.com/blog/blogdown-netlify-new-post-workflow/).
:::
...
## new page folders
The `home` folder is the only "separate page"-type folder here right now --- the Academic Theme by default generates a 1-page website --- but you can [add new folders for new website pages](#separate-pages).  Inside `content/home`, you'll see a file `index.md`; open it up.  It should say:
...
4. Otherwise, take a look at [Alison Hill's troubleshooting guide](https://alison.rbind.io/post/2019-03-04-hugo-troubleshooting/), the [resources](#resources) linked above, Google, or other peoples' website repos!
    + [my repo](https://github.com/akseong/website)
    + [Nan-Hung Hsieh's repo](https://github.com/rbind/nanhung), and [website](https://nanhung.rbind.io/)
        - doesn't deviate from template too much
    + [Alison Hill's repo](https://github.com/rbind/apreshill/), and [website](https://alison.rbind.io/);
        - more advanced customization
    + [Yihui Xie's repo](https://github.com/rbind/yihui), and [website](yihui.org)
        - looks simple, but very advanced customization
...
There are 3 main files to edit:  
...
In your root directory, change the following fields in `config.yaml`:  
  ...
  + `ignoreFiles:` Yihui Xie [recommends specifying some regex patterns](https://bookdown.org/yihui/blogdown/configuration.html#options).  If you've been following this guide so far, you have a `config.yaml` file instead of a `config.toml` file (as in the link).  The only thing you need to do in order to match Yihui Xie's recommendations is to add the following to the default list under `ignoreFiles`
...
:::fyi
Some guides recommend including `"_files$"` in `ignoreFiles`, __but this will break plots in .Rmd-generated posts__.  
...
Almost done.  Add the lines (I put these right beneath `baseurl:`):
```{r}
relativeURLs: yes
canonifyURLs: yes
```
...
Last, beneath `permalinks`, I suggest adding `post: /:year-:month-:day/:slug/` (follow indents; this modifies post urls, e.g. ".../2020-12-28/make-website/" rather than ".../post/make-website/").
...
Generating a table of contents using the built-in shortcodes or .Rmd header puts the table of contents first.  This is usually a good thing, except it can take up your entire post preview (i.e. the text you see below the post titles [here](/post_landing/).  
