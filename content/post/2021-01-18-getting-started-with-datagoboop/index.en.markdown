...
The main difference between these packages and `datagoboop` is that `datagoboop` includes several "built-in" functions for sonification, but also allows users to build "audio plots" piece by piece by providing a higher level of control and modular functions.
